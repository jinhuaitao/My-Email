...
您需要一个 R2 存储桶来存储邮件和系统配置，以及一个 Worker 服务来运行前端代码和邮件处理逻辑。
...
# 3. 接收邮件的 Worker 路由 (关键设置)
[[rules]]
type = "email"
pattern = "inbox@example.com" # 替换为你用于接收邮件的地址，例如 mail@yourdomain.com
receiver_tag = "email_handler" # 必须与步骤三中 Email Routing 的名称匹配
步骤二：部署 Worker
使用 Wrangler 部署您的 Workers 服务。

Bash

wrangler deploy
部署成功后，Worker 就会被激活，但它还无法接收邮件。
